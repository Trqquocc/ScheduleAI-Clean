/**
 * Calendar Module v6.3 – FULLY REWRITTEN & BUG-FREE
 * + Conflict detection
...
(function () {
  ...
  const CalendarModule = {
    ...
    async init() {
      ...
      console.log("Khởi tạo CalendarModule v6.3...");
      this.initPromise = this._initInternal();

      try {
        await this.initPromise;
        this.isInitialized = true;
        console.log("CalendarModule v6.3 khởi tạo thành công!");
      } catch (err) {
    ...
    async _initInternal() {
      ...
      // Setup draggable & navbar sau khi calendar đã render
      setTimeout(() => {
      ...
      this.calendar = new FullCalendar.Calendar(el, {
        ...
        eventReceive: (info) => {
          if (this.hasTimeConflict(info.event)) {
            Utils?.showToast?.("Đã có lịch vào thời gian này!", "error");
            info.event.remove();
            return;
          }
    ...
    async handleEventReceive(info) {
      try {
        ...
        if (res.success) {
          ...
          Utils.showToast?.("Đã thêm vào lịch!", "success");
        } else throw new Error();
      } catch (err) {
        info.event.remove();
        Utils.showToast?.("Lỗi thêm sự kiện", "error");
      }
    },
    ...
    async handleEventUpdate(info) {
      try {
        ...
        Utils.showToast?.("Đã cập nhật lịch", "success");
      } catch (err) {
        info.revert();
        Utils.showToast?.("Lỗi cập nhật", "error");
      }
    },
          ...
          <div class="bg-white rounded-xl shadow-2xl w-full max-w-md mx-4">
            <div class="p-6">
              ...
              <div class="space-y-3 text-sm mb-6">
                <div class="flex"><span class="w-28 text-gray-600 font-medium">Bắt đầu:</span><span>${startStr}</span></div>
                <div class="flex"><span class="w-28 text-gray-600 font-medium">Kết thúc:</span><span>${endStr}</span></div>
                <div class="flex"><span class="w-28 text-gray-600 font-medium">Ghi chú:</span><span>${
                  p.note || "Không có"
                }</span></div>
                <div class="flex"><span class="w-28 text-gray-600 font-medium">Trạng thái:</span>
                  <span class="${
              ...
              <div class="p-4 bg-gray-50 rounded-lg mb-6">
                ...
                <p class="text-sm text-gray-500 mt-2 ml-8">Sẽ được lưu vào lịch sử & thống kê</p>
              </div>

              <div class="flex justify-end gap-3">
                <button id="closeEventDetail" class="px-5 py-2.5 border border-gray-300 rounded-lg hover:bg-gray-50">Đóng</button>
                <button id="saveEventStatus" class="px-5 py-2.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 font-medium">Lưu thay đổi</button>
              </div>
            </div>
          </div>
        </div>`;
    ...
    setupExternalDraggable() {
      ...
      this.draggableInstance = new FullCalendar.Draggable(taskList, {
        ...
        eventData: (el) => ({
          title: el.dataset.taskTitle || el.textContent.trim() || "Công việc",
  ...
  console.log("CalendarModule v6.3 đã sẵn sàng!");
})();
